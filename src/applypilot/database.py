...
def _pg() -> bool:
    import os
    return os.environ.get("APPLYPILOT_DB_URL", "").startswith(("postgres://", "postgresql://"))


def get_connection(db_path: Path | str | None = None) -> sqlite3.Connection:
    """Get a thread-local cached connection.

    With APPLYPILOT_DB_URL set (postgresql://...), returns a PgConnection
    from dbcompat — one shared database for all machines. Otherwise the
    local SQLite file (WAL mode, sqlite.Row factory).
    """
    from applypilot.dbcompat import backend, connect
...
def init_db(db_path: Path | str | None = None) -> sqlite3.Connection:
    ...
    """
    from applypilot.dbcompat import backend
    kind, target = backend()
    if kind == "postgres":
        conn = get_connection()
    else:
        path = db_path or target
    ...
    id_col = ("id          BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY"
              if _pg() else "id          INTEGER PRIMARY KEY AUTOINCREMENT")
    conn.execute(f"""
...
def ensure_columns(conn: sqlite3.Connection | None = None) -> list[str]:
    ...
    existing = {row[1] for row in conn.execute("PRAGMA table_info(jobs)").fetchall()} if not _pg() else \
        {(row[0].decode() if isinstance(row[0], (bytes, bytearray)) else row[0]) for row in conn.execute(
            "SELECT column_name::text FROM information_schema.columns WHERE table_name = 'jobs'"
        ).fetchall()}
    added = []
